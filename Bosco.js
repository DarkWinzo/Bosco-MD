...
module.exports = bosco = async (bosco, mek) => {
	try {
        ...
        const isStctext = isStcMedia && content.includes("quotedMessage")
		...
		const catl = (text) => {
             res = bosco.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 400, "message": text, "footerText": "*DarkWinzo*", "thumbnail": dfrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             bosco.relayWAMessage(res)
        }
       const catlo = (text) => {
             res = bosco.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 70000, "message": text, "footerText": "Made With DarkWinzo", thumbnail: fs.readFileSync('./ds.jpg'), "surface": 'CATALOG' }}, {quoted:ftroli})
             bosco.relayWAMessage(res)
        }
        const grupinv = (text) => {
        	grup = bosco.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/IWvaZWObbJK0Dr64WQdGqO', "groupName": `Bosco Family`, "footerText": "*DarkWinzo*", "jpegThumbnail": dfrply, "caption": text}}, {quoted:fvideo})
		...
		const DarkWinzo = await getBuffer(pporang)
        ...
        const reply = (text) => {
	      bosco.sendMessage(from, text, text, { thumbnail: DarkWinzo, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: false, "externalAdReply": { "title": `${' '}Sᴜʙsᴄʀɪʙᴇ Yᴛ : DarkWinzo${''}${''}`, "body": `Gʀᴏᴜᴘ Assɪsᴛᴇɴᴛ Bᴏᴛ`, "previewType": 'PHOTO', "thumbnailUrl": `${''}`, "thumbnail": DarkWinzo, "sourceUrl": `${''}`}},})
        }
        const sendMess = (hehe, text) => {
           bosco.sendMessage(hehe, text, text)
        }
        const mentions = (text, memberr, id) => {
           (id == null || id == undefined || id == false) ? bosco.sendMessage(from, {text: text.trim(), jpegThumbnail: fs.readFileSync('./media/wpmobile.jpg')}, extendedText, { sendEphemeral: true, contextInfo: { "mentionedJid": memberr } }) : bosco.sendMessage(from, {text: text.trim(), jpegThumbnail: fs.readFileSync('./media/wpmobile.jpg')}, extendedText, { sendEphemeral: true, quoted: mek, contextInfo: { "mentionedJid": memberr } })
        }
        const sendText = (from, text) => {
           bosco.sendMessage(from, text, MessageType.text)
        }
        const textImg = (text) => {
           return bosco.sendMessage(from, text, text, {quoted: mek, thumbnail: fs.readFileSync('./media/wpmobile.jpg')})
        }
        
        const fakestatus = (text) => {
            return bosco.sendMessage(from, text, text, {
                quoted: {
                    key: {
                        fromMe: false,
        ...
        const fakethumb = (text, yes) => {
            return bosco.sendMessage(from, text, image, {thumbnail:fs.readFileSync('./stik/fake.jpg'),quoted:mek,caption:yes})
        }
        const fakegroup = (text) => {
            return bosco.sendMessage(from, text, text, {
                quoted: {
                    key: {
                        fromMe: false,
      ...
      const fgclink = (text) => {
            bosco.sendMessage(from, text, text, {
                quoted: {
	                key: {
		               fromMe: true,
...
remoteJid :"status@broadcast" }: {})
            ...
            const jmo = moment.tz('Asia/Colombo').format('DD/MM/YYYY')
            const jmn = moment.tz('Asia/Colombo').format('hh:mm')
				let d = new Date
				let locale = 'en'
...
const fvid = {
	 ...
	 message: { 
        ...
        //FAKEREPLY PRODUCT
            ...
            //FAKE TEXT
            const ftext = {
                  key: {fromMe: false,participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "16505434800@s.whatsapp.net" } : {})},message: { "extendedTextMessage": {"text": `▢ ${fake} ▢`,"title": `Hmm`,'jpegThumbnail': DarkWinzo}}}
            //FAKE LIVE ACTION
            const floc2 = {
            ...
            //FAKEREPLY TROLI ADDED BY DARKWINZO
            const ftroli = {
                  key: {participant: "0@s.whatsapp.net", ...(from ? { groupJid: "120363042182512544@g.us" } : {})},message: { "orderMessage": { "itemCount" : '299992', "status": '1', "surface": '1', "message": `${fake}`, "orderTitle": 'Bang', "thumbnail": DarkWinzo, "sellerJid": '0@s.whatsapp.net'}}}
            //FAKEREPLY VIDEO
            const fvideo = {
...
// AUTO
			...
			for (let anjh of imagi){
				if (budy === anjh){
					...
					bosco.sendMessage(from, result, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})}, message: { orderMessage: { itemCount: 500, status: 200, thumbnail: fs.readFileSync('./ds.jpg'), surface: 200, message: `${fake}`, orderTitle: `Made By DarkWinzo`, sellerJid: '0@s.whatsapp.net'}}}})
					}
			}
      ...
      const sendWebp = async(to, url) => {
       ...
       const sendMediaURL = async(to, url, text="", mids=[]) =>{
           ...
           bosco.sendMessage(to, media, type, {quoted: mek, "externalAdReply": { "title": `${' '}Sᴜʙsᴄʀɪʙᴇ Yᴛ : DarkWinzo${''}${''}`, "body": `Gʀᴏᴜᴘ Assɪsᴛᴇɴᴛ Bᴏᴛ`, "previewType": 'PHOTO', "thumbnailUrl": `${''}`, "thumbnail": DarkWinzo, "sourceUrl": `${''}`}, mimetype: mime, caption: text, thumbnail: Buffer.alloc(0), contextInfo: {"mentionedJid": mids}})
      ...
      const sendFileFromUrl = async(link, type, options) => {
          ...
          if (!packname) packname = '!Dark'; if (!author) author = 'Winzo';author = author.replace(/[^a-zA-Z0-9]/g, '');	
...
// Anti link
        if (budy.includes("https://chat.whatsapp.com/")) {
        	if (!mek.key.fromMe){
				...
				reply('Link terdetexti, Auto kick!')
       ...
       if (isGroup && isAntiLink && !isOwner && !isGroupAdmins && isBotGroupAdmins){
        ...
        bosco1 = await bosco.prepareMessage(from, DarkWinzo, location, {thumbnail: DarkWinzo})
...
//------------------< Sticker Cmd >-------------------
       ...
       case 'delcmd':
              ...
              break
       case 'listcmd':
              let textnyee = `*「 LIST STICKER CMD 」*`
              let cemde = [];
              for (let i of _scommand) {
              cemde.push(i.id)
              textnyee += `\n\n➸ *ID :* ${i.id}\n➸ *Cmd* : ${i.chats}`
               }
              mentions(textnyee, cemde, true)
              break
//------------------< Bot Owner >-------------------
       ...
       case 'group':
        rows = [
          ...
          section = [{title: "Sᴜʙsᴄɪʙᴇ Yᴛ DᴀʀᴋWɪɴᴢᴏ Fᴏʀ Mᴏʀᴇ Uᴘᴅᴀᴛᴇs", rows: rows}]
...
break
      ...
      case "setthumb":
        ...
        break
        case 'exif':
             if (args.length == 0) return reply(`Ex:- ${prefix}exif DarkWinzo`)
             if (!isOwner) return  reply(mess.only.owner)
     ...
     case 'seenby':
             ...
             text += '@' + i.jid.split('@')[0] + '\n'
             text += `> ` + moment(`${i.t}` * 1000).tz('Asia/Kolkata').format('DD/MM/YYYY hh:mm:ss') + '\n\n'
             tagg.push(i.jid)
}
             text += `*• Delivered to :*\n\n`
...
*Sɪᴢᴇ :* ${res[0].size}`
      ...
      case 'hidetag':
       ...
       case 'DarkWinzo':
              if (!q) return
              qq = q.toUpperCase()
              awikwok = `${qq} ${qq} ${qq} ❤️ ❤️ ❤️ WANGY WANGY WANGY HU HA HU HA HU HA, Pepe Poli Ahn ${qq} Pepe Killadi Ahnu ${qq} AAAAAAAAH ~ Her hair.... aaah I want to stroke her hair too ~~ AAAAAH ${qq} first time out in anime too cute ❤️ ❤️ ❤️ so AAAAAAAAH ${qq} AAAAAA LUCCUUUUUUUUUUUUUU........... ${qq} AAAAAAAAAAAAAAAAAAAAAGH ❤️ ❤️ ❤️what ? ${qq} is not real? Just HELL you say? no no no no no no no no no no i dont believe it i dont KNOW THAT HE IS REAL!! I DON'T CARE ABOUT THE REALITY, I DON'T CARE. ❤️ ❤️ ❤️ ${qq} I ... ${qq} on my laptop looking at me, ${qq} .. do you trust me? aaaaaaaaaaah thanks ${q} I don't want to give up ${qq} aaaaaah ❤️ ❤️ ❤️ YEAAAAAAAAAAAH I STILL HAVE ${qq} IT'S NOT THE SAME AAAAAAAAAAAAAAH`
              reply(awikwok)
              break
        case 'jadibot':
              if (!isOwner) return
...
name: nama,
address: impostor,
jpegThumbnail: DarkWinzo}, MessageType.liveLocation, {quoted:floc2})
break
       case 'getdeskgc':
				if (!isGroup) return reply(mess.group)
				anu = from
           ...
           case 'chat':
            ...
            if (args[0].startsWith('+94')) return reply('*Prefix number with 94*')
			if (args.length < 1) return reply(`Use ${prefix}chat 94xxxx|text`)
            var pc = body.slice(6)
      ...
      case 'photo':
              ...
              bosco.sendMessage(from, buffer, image, {quoted: mek, thumbnail:DarkWinzo, caption: 'By Bosco'})
              fs.unlinkSync(ran)
})
              break
//------------------< Public/Self >-------------------
...
//------------------< Downloader/And Search Media >-------------------
      ...
      case 'more':
        ...
        case 'cekapikey':
               ...
               bosco.sendMessage(from, text, text, {quoted: mek})
               break
       case 'pinterest':
         case 'pin':
              if (args.length < 1) return reply(`${prefix}DarkWinzo`)
...
`.trim()
     ...
     case 'googleimage':
              ...
              reply(mess.wait)
              text = args.join(' ')
              res = await _gis(text, google)
              function google(error, result){
              if (error){ return reply('*Error An Error Occurs Or No Result Found*')}
              else {
              gugIm = result
              random =  gugIm[Math.floor(Math.random() * gugIm.length)].url
              sendFileFromUrl(random, image, {quoted: mek, thumbnail: Buffer.alloc(0), caption: `*Search Results From :* ${text}`})
              }
              }
             break
      case 'tiktokdl':
      ...
      case 'tiktokmusic': 
      case 'tiktokaudio':
             if (args.length == 0) return reply(`Example: ${prefix + command} https://vt.tiktok.com/Link`)
             ini_link = args[0]
...
`
              ...
              bosco.sendMessage(from, ini_buffer, image, { quoted: mek, caption: text })
              } catch {
...
`
 ...
 menumessage = { contentText: `${text}`, footerText: `Sᴜʙsᴄʀɪʙᴇ Yᴛ DᴀʀᴋWɪɴᴢᴏ`, buttons: menubutton, headerType: 6, locationMessage: bosco2.message.locationMessage}
...
case 'attp':
       ...
       case 'sgif':
              ...
              if (err) return reply(`${err}`)
              exec(`webpmux -set exif ${addMetadata('DarkWinzo')} ${ron} -o ${ron}`, async (error) => {
              if (error) return reply(`${error}`)
...
case 'emoji':
			...
			text = `${emoji.images[4].url}`
    		sendStickerFromUrl(from,`${text}`)	
    		console.log(text)
   			})
    		break
...
//------------------< Storage >-------------------
	 ...
	 case 'delvn':
				...
				case 'listbgm':
					...
					text += `\n*Tᴏᴛᴀʟ : ${vien.length}*\n\n_🍁_`
					bosco.sendMessage(from, text.trim(), extendedText, { caption: 'text', "contextInfo": { text: 'text', sendEphemeral: true, "externalAdReply": { "title": `${' '}DᴀʀᴋWɪɴᴢᴏ${''}${''}`, "body": ``, "previewType": 'PHOTO', "thumbnailUrl": `${'https://i.ibb.co/vkkcm0L/034c588fd8d5.jpg'}`, "thumbnail": '', "sourceUrl": `${'https://wa.me/+94775200935'}`}},quoted: ftext})
					break
				case 'addimage':
				    if (!isOwner) return
				...
				case 'listimage':
					...
					bosco.sendMessage(from, text.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": imagi } })
					break
              case 'addsticker':
                    if (!isOwner) return
					if (!isQuotedSticker) return reply('*Reply To Sticker*')
...
//------------------< Group >-------------------
       ...
       case 'block':
        ...
        case 'fetch': //ambil dari nuru
               if (!/^https?:\/\//.test(q)) return reply('Prefix *URL* with http:// atau https://')
               res = await fetch(q)
...
//------------------< Bosco-MD Maker Menu >-------------------
                ...
                case 'horrorblood':
                case 'thunder':
                    if (args.length == 0) return reply(`Example: ${prefix + command} DarkWinzo hehe`)
                    ini_txt = args.join(" ")
...
text = args.join(' ')
res = await wikiSearch(text).catch(e => {
...
text = mek.message.extendedTextMessage.contextInfo.quotedMessage.conversation
...
if (Number(oi2) >= 50) return reply('*Most!*')
	if (!Number(oi2)) return reply('*The number must be a number!*')
     ...
     case 'mute':
                ...
                break
     case 'tts':
					if (args.length < 1) return bosco.sendMessage(from, `Where's the text? example : ${prefix}tts id yamate kudasai`, text, { quoted: mek })
				   const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return bosco.sendMessage(from, `Where's the text? example : ${prefix}tts id yamate kudasai`, text, { quoted: mek })
     ...
     case 'tagme':
				...
				if (!isOwner) return 
					...
					if (err) return bosco.sendMessage(from, `root @DarkWinzo:~ ${err}`, text, { quoted: mek })
                 break
              case 'swm':
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
							ppp = `${args.join(' ')}`
