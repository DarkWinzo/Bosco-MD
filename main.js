...
nocache('../message/help.js', module => console.log(color('[WATCH]', 'cyan'), color(`'${module}'`, 'green'), 'File is updated!'))
	...
	console.log(color(figlet.textSync('Bosco-MD ᴰᵃʳᵏᵂⁱⁿᶻᵒ', {
		font: 'Standard',
	...
	console.log(color('[DarkWinzo]', 'cyan'), color('WELCOME', 'white'))
	console.log(color('[Bosco-MD]', 'cyan'), color('BOT RUNNING', 'yellow'))
	bosco.browserDescription = ["SUBSCRIBE YT - [DarkWinzo]", "firefox", "3.0.0"];

	// Menunggu QR
	bosco.on('qr', () => {
		console.log(color('[', 'white'), color('!', 'red'), color(']', 'white'), color('Please Scan Bosco-MD qr code'))
	})

	// Menghubungkan
	fs.existsSync(`./DarkWinzo.json`) && bosco.loadAuthInfo(`./DarkWinzo.json`)
	bosco.on('connecting', () => {
	...
	fs.writeFileSync(`./DarkWinzo.json`, JSON.stringify(bosco.base64EncodedAuthInfo(), null, '\t'));
...
`
   ...
   remoteJid: "0@s.whatsapp.net"},message: {"groupInviteMessage": {"groupJid": "6288213840883-1616169743@g.us","inviteCode": "mememteeeekkeke","groupName": "Alphabot", "caption": `🌹ＤＡＲＫＷＩＮＺＯ 🌹`, 'jpegThumbnail': fs.readFileSync(`ds.jpg`)}}}
